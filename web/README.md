...
Or from `web/` only (you still need root `pnpm install` once for workspace deps):
...
**Zero configuration** runs heuristics-only. Optional env in `web/.env.local`:
...
Deploy from the **repo root** on Vercel (see root `README.md` and [`vercel.json`](../vercel.json)). Do not set the Vercel root to `web/` alone — the build imports `../src/core`.
